...
- 执行摘要由 `_executive_summary` 驱动，当前为**4 个子板块独立调用**：`🔑 本季核心主题`、`⚡ 重要事件`、`💬 管理层关键信号`、`⚠️ 主要风险`（`📊 财务快照`子板块已移除）。
- 每个执行摘要子板块都有独立 prompt 约束与截断续写；并在写入前执行后处理（白名单过滤、禁用标题裁剪等）。
- 执行摘要输入侧会先对白名单做预过滤：`step4_text` / `step5_text` 在喂入 prompt 前即经过 `_filter_by_ticker_whitelist(...)`。
- `⚡ 重要事件` 与 `💬 管理层关键信号` 在落盘前额外执行 `_filter_non_sector_content(...)`，只按明确 ticker 引用位过滤非监控标的，减少误杀 `EBITDA`/`GDP` 一类缩写。
- **「相对强弱」排名表**继续由代码固定公司行（有营收等财务数据的标的），LLM 仅返回 JSON 填充优势/风险；解析失败时降级为空表占位，不阻断整份报告。

### 5.4 Step2 / Step3 / Step4 / Step5 要点

- **Step2**：地理或分部收入展示前，与已校验总营收做 **覆盖率** 核对；过低覆盖会弱化或跳过易误导的地理表表述。
- **Step3**：板块总结由 2 次调用组成（`【行业判断】` + `【战略方向】`），各自 `max_tokens=1200`；写入前先过 `_filter_non_sector_content(...)` 与 `_strip_unauthorized_sections(...)`，再拼接。
- **Step4**：板块总结由 3 次调用（`【AI部署与替代进展】`、`【人员与组织变动】`、`【营收与盈利关键数据】`）组成；拼接后立即执行 `_sanitize_bracket_tickers(...)` + `_filter_by_ticker_whitelist(...)`。
- **Step5**：板块总结由 2 次调用（`【重大投资与收购】` + `【重大战略合作与其他动态】`）组成；输出侧使用模块级 `_sanitize_bracket_tickers(...)` + `_filter_by_ticker_whitelist(...)` 处理非监控标的。  
  对 LLM 偶发 `[EL\n]` 这种断裂 bracket ticker，可在 sanitize 阶段自动归并为 `[EL]`。
...
- **执行摘要内容异常**：先看 4 个子板块是否命中各自续写与后处理；`⚡/💬/⚠️` 板块有额外过滤（ticker 白名单 + 未授权段落裁剪）可能导致部分行被移除。  
- **排名表异常**：查日志中 ranking JSON 解析与降级分支；空优势/风险单元格属于兜底行为。  
- **前端调 API 失败**：核对 Streamlit 端口是否在 FastAPI CORS 白名单内。  
- **SEC / FMP 403、402**：检查 User-Agent、密钥权限与 FMP URL 版本是否与客户端一致。  
- **日志中出现 `Ticker白名单过滤丢弃整行`**：先确认是否真为非监控标的；当前过滤只对明确 ticker 引用位生效，若仍误杀可补充格式规则或放宽调用点。  
