...
- 执行摘要由 `_executive_summary` 驱动，已改为**分板块独立调用**：`📊 财务快照`、`🔑 本季核心主题`、`⚡ 重要事件`、`💬 管理层关键信号`、`⚠️ 主要风险` 分别单独生成并拼接（板块间 `---` 分隔）。
- 每个执行摘要子板块都有独立 prompt 约束与截断续写；输出格式统一为“先 2-4 句段落总结，再按小标题分组 bullet 支撑数据”。
- `⚡ 重要事件` 与 `💬 管理层关键信号` 在落盘前会做**非监控标的白名单过滤**，剔除含非本 sector ticker 的行。
...
- **Step3**：板块总结由 1 次调用拆为 2 次（`【行业判断】` + `【战略方向】`），各自独立续写后拼接；要求只提炼跨 2 家以上公司的共同信息并附来源。
- **Step4**：板块总结由 1 次调用拆为 3 次（`【AI部署与替代进展】`、`【人员与组织变动】`、`【营收与盈利关键数据】`），各自独立续写后拼接，减少单次输出截断。
- **Step5**：板块总结由 1 次调用拆为 2 次（`【重大投资与收购】` + `【重大战略合作与其他动态】`）并拼接；Prompt 侧强调 **监控名单内 ticker**；输出侧 `_filter_step5_sector_signal` 做白名单过滤：整行仅非法标的时删除；合法与非法混排时对非法 `[TICKER]` / 词边界 ticker **脱敏为** `〔非本板块标的〕`。  
